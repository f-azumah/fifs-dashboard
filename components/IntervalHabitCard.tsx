...
export default function IntervalHabitCard({
  ...
  return (
    <div className="group rounded-lg border border-black/10 bg-white/60 p-3 flex flex-col gap-2">
      <div className="flex items-start justify-between">
        ...
        <button
          ...
          className="opacity-0 group-hover:opacity-100 text-black/30 hover:text-black/60 text-xs"
      ...
      <div className="h-1.5 rounded-full bg-black/10 overflow-hidden">
        <div
          className={`h-full rounded-full ${overdue ? "bg-red-400" : "bg-emerald-500"}`}
      ...
      <button
        disabled={isPending}
        onClick={() => startTransition(() => logHabitCompletionToday(habit.id))}
        className="text-xs px-2 py-1.5 rounded-md bg-black/90 text-white hover:bg-black self-start"
      >
        Mark done today
      </button>
    </div>
